...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text): 
        return self.word in self.makeWordList(text)
        
    def makeWordList(self, text):
        text_copy = ''
        # replace all punctuation with space and lower the case
        for char in text:
            if char in string.punctuation:
                char = ' '
            text_copy += char.lower()
        # return list of splitted words
        return text_copy.split(' ')
    ...
    for story in stories:
        for trigger in triggerlist:
            if trigger.evaluate(story):
                filtrdStories.append(story)
                break
    return filtrdStories
...
def main_thread(master):
    ...
    try:
        ...
        t1 = TitleTrigger("")
        t2 = SubjectTrigger("elections")
        t3 = PhraseTrigger("Bernie Sanders")
        t4 = OrTrigger(t2, t3)
        triggerlist = [t1, t4]
        
        # After implementing makeTrigger, uncomment the line below:
        #triggerlist = readTriggerConfig("/Users/makarymalinouski/Dropbox/MIT CS6.00x/Week7/ProblemSet7/triggers.txt")
