...
#random.seed()  
#viruses = [ SimpleVirus(0.87, 0.85), SimpleVirus(0.74, 0.89), SimpleVirus(0.06, 0.92), SimpleVirus(0.59, 0.79) ]
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    pylab.title("Virus Population by Time")
...
class ResistantVirus(SimpleVirus):
    ...
    def isResistantTo(self, drug):
        ...
        try:
            ...
            return None#print "This drug is not in resistance list"
...
#v = ResistantVirus(0.5, 0.5, {"a": True, "b": True}, 0.1)
#try:
#    v2 = v.reproduce(0.5, ["a", "b"])
#    print v2.getMutProb()
#except NoChildException:
#    print "Exception"
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        integer)
        """
        viruses_copy = list(self.viruses)
        for virus in self.viruses:
            if virus.doesClear():
                viruses_copy.remove(virus)
                
        self.viruses = list(viruses_copy)
        popDensity = float(self.getTotalPop())/self.getMaxPop()
        
        for virus in self.viruses:
            try:    
                viruses_copy.append(virus.reproduce(popDensity, self.drugs))
            except NoChildException:
                pass
        self.viruses = list(viruses_copy)
        return self.getTotalPop()
            
#rv = ResistantVirus(0.8, 0.2, {"a":False, "b":True}, 0.2)
#rv2 = ResistantVirus(0.7, 0.3, {"a":False, "b":False, "c":False}, 0.1)
#tp = TreatedPatient([rv], 10)
#tp.addPrescription("a")
#print rv.isResistantTo("a") 
#print tp.getResistPop(["a"])
#print tp.getTotalPop()
#print tp.update()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for i in range(numTrials):
        ...
        tPatient = TreatedPatient(viruses, maxPop)
        
        for i in range(150):
            virusPop[i] += tPatient.update()
            resVirusPop[i] += tPatient.getResistPop(["guttagonol"])
    ...
    pylab.plot(range(300), virusPop)
    pylab.plot(range(300), resVirusPop)
    pylab.title("Virus Population Over Time Before And After Introducing Drug at 150 ts")
    pylab.xlabel("Timesteps")
